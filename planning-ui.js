(function initializeMerPlanning(root) {
  ...
  let dialog, tab = 'fire', owner = null, editingRenewalId = null, renewalOriginal = null, fireDraft = null, lastReminderSignature = '';
  ...
  function close() { if (dialog?.open) (root.MerEnterpriseBridge?.closeModal || (node => node.close()))(dialog); }
  ...
  function ensureDialog() {
    if (dialog) return;
    dialog = document.createElement('dialog'); dialog.id = 'planningModal'; dialog.className = 'modal planning-dialog'; dialog.setAttribute('aria-labelledby', 'planningTitle');
    dialog.innerHTML = `<header class="planning-head"><div><span id="planningProfile"></span><h2 id="planningTitle"></h2></div><button type="button" class="icon-button" data-planning-close aria-label="${say('Zatvori','Close')}">×</button></header><nav class="planning-tabs" aria-label="${say('Planiranje','Planning')}"><button type="button" data-planning-tab="fire">FIRE</button><button type="button" data-planning-tab="renewals">${say('Zaštita pretplata','Subscription safeguard')}</button></nav><div class="planning-body"><section id="planningFire"></section><section id="planningRenewals" hidden></section></div><footer class="planning-footer"><button type="button" class="secondary-button" data-planning-close>${say('Zatvori','Close')}</button><button type="button" class="primary-button" id="saveFirePlan">${say('Spremi pretpostavke','Save assumptions')}</button></footer>`;
    document.body.append(dialog);
    dialog.querySelectorAll('[data-planning-close]').forEach(button => button.addEventListener('click', close));
    dialog.querySelectorAll('[data-planning-tab]').forEach(button => button.addEventListener('click', () => {tab = button.dataset.planningTab; renderDialog();}));
    ...
    el('saveFirePlan').addEventListener('click', () => {
      ...
      saveMutation('fire-assumptions-save', current => {current.enterprise ||= {}; current.enterprise.firePlan = {...fireDraft};});
  ...
  function taxExplanation() {
    const current=profile(), tax=listGoals(current).filter(goal=>goal.taxVault).reduce((sum,goal)=>sum+(Number(goal.current)||0),0);
    return `<aside class="planning-tax"><strong>${say('Porezni trezor','Tax vault')}: <span data-money>${money(Math.round(tax*100))}</span></strong><p>${say('Pri oporezivoj B2B uplati od bruto 1.250 € pričuva iznosi 250 €: 25% na neto osnovicu, odnosno bruto × 25/125. Trezor se osvježava s izvornom uplatom.','A taxable gross B2B receipt of €1,250 reserves €250: 25% of net, or gross × 25/125. The vault updates with its source payment.')}</p></aside>`;
  }
  const listGoals=current=>Array.isArray(current.goalBuckets)?current.goalBuckets:[];
  function bindRenewalForm() {
    el('renewalForm').addEventListener('submit', event => {
      ...
      editingRenewalId=null;renderRenewals();showToast(say('Podsjetnik je spremljen.','Reminder saved.'));
    });
  ...
  function renderDialog() {
    if(!dialog)return;
    el('planningProfile').textContent=profile().accountName||activeId();el('planningTitle').textContent=tab==='fire'?say('Financijska neovisnost · FIRE','Financial independence · FIRE'):say('Obnove bez iznenađenja','Renewals without surprises');
    el('planningFire').hidden=tab!=='fire';el('planningRenewals').hidden=tab!=='renewals';el('saveFirePlan').hidden=tab!=='fire';
    dialog.querySelectorAll('[data-planning-tab]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.planningTab===tab)));
    if(tab==='fire')renderFire();else renderRenewals();
  }
  function open(which='fire') {
    if(root.MerEnterpriseSecurity?.isLocked?.()||document.getElementById('appShell')?.hidden)return;
    ensureDialog();tab=which==='renewals'?'renewals':'fire';owner=activeId();editingRenewalId=null;fireDraft=defaultFire();renderDialog();(root.MerEnterpriseBridge?.openModal||(node=>node.showModal()))(dialog);
  }
  function render() {
    if(typeof appState==='undefined')return;
    if(dialog?.open){if(owner!==activeId()){close();owner=null;editingRenewalId=null;renewalOriginal=null;fireDraft=null;}else renderDialog();}
  ...
  root.MerPlanningUI=Object.freeze({open,render,reminders:()=>P.dueReminders(profile(),options()),downloadCalendar});
