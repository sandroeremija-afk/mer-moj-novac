(function initializePremiumFeatures() {
  Object.assign(translations.hr, {
    ...
    smartImport:'PAMETNI UVOZ', importHistory:'Uvezite prihode i troškove', chooseImportFile:'Odaberite CSV ili Excel datoteku', importFileHint:'Prepoznajemo datum, opis, iznos, vrstu i kategoriju. Ništa se ne sprema prije potvrde.', chooseFile:'Odaberi datoteku', loadLargeSample:'Učitaj testni primjer (520 redaka)', include:'Uključi', date:'Datum', description:'Opis', type:'Vrsta', previous:'Prethodno', next:'Sljedeće', applyToIncluded:'Primijeni na uključene', confirmImport:'Potvrdi uvoz', importSummary:'{ready} spremno · {duplicates} duplikata · {invalid} nevaljanih', importPage:'Stranica {page} od {pages}', importFinished:'Uvezeno je {count} transakcija; {duplicates} duplikata je preskočeno.', importFailed:'Datoteku nije moguće pročitati. Provjerite stupce i format.', excelUnavailable:'Excel modul nije učitan. Pokušajte ponovno ili upotrijebite CSV.',
  ...
  Object.assign(translations.en, {
    ...
    smartImport:'SMART IMPORT', importHistory:'Import income and expenses', chooseImportFile:'Choose a CSV or Excel file', importFileHint:'We detect date, description, amount, type and category. Nothing is saved before confirmation.', chooseFile:'Choose file', loadLargeSample:'Load test sample (520 rows)', include:'Include', date:'Date', description:'Description', type:'Type', previous:'Previous', next:'Next', applyToIncluded:'Apply to included', confirmImport:'Confirm import', importSummary:'{ready} ready · {duplicates} duplicates · {invalid} invalid', importPage:'Page {page} of {pages}', importFinished:'Imported {count} transactions; skipped {duplicates} duplicates.', importFailed:'The file could not be read. Check its columns and format.', excelUnavailable:'The Excel module is not loaded. Try again or use CSV.',
  ...
  function renderGoals() {
    ...
    $('#goalBucketGrid').innerHTML=goals.map(goal=>{const result=MerCore.validateSavingsGoal(goal),percent=Math.round(result.percent||0);return `<article class="goal-bucket-card"><div class="goal-bucket-head"><span class="goal-bucket-icon">${escapeHtml(goal.icon||'◎')}</span><div><strong>${escapeHtml(goal.name)}</strong><small>${goal.primary?t('primaryGoal'):(goal.dueDate?t('goalDue',{date:preferredDate(goal.dueDate)}):t('goalNoDate'))}</small></div><button type="button" class="icon-button small" data-edit-goal="${goal.id}" aria-label="${t('editGoal')}"><svg aria-hidden="true"><use href="#icon-edit"></use></svg></button></div><div class="goal-bucket-values"><strong>${currency(goal.current,true)}</strong><span>${t('goalOf',{target:currency(goal.target,true)})}</span></div><div class="goal-track"><span style="width:${percent}%"></span></div><div class="goal-bucket-foot"><small>${t('goalRemaining',{amount:currency(result.remaining,true)})}</small><b>${percent}%</b></div></article>`;}).join('');
