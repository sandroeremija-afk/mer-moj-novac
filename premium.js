(function initializePremiumFeatures() {
  ...
  Object.assign(translations.hr, {
    ...
    settingsIntro:'Prikaz, podaci, sigurnost i pravila.',
    settingsIntroClean:'Prikaz, podaci, sigurnost i pravila.',
    baseCurrencyHint:'Mijenja oznaku valute i oblik prikaza; ne preračunava iznose po tečaju.',
    baseCurrencyInfo:'Informacije o osnovnoj valuti',
    timezoneHint:'Određuje današnji datum i vrijeme prikaza; uneseni datumi transakcija ostaju isti.',
    settingsLanguageHint:'Mijenja jezik sučelja, ne podatke ni nazive vaših kategorija.',
    hideBalancesHint:'Zamagljuje iznose na zaslonu, ali ih ne šifrira. Prečac: Ctrl / ⌘ + Shift + H.',
    automationSubtitle:'Pravilo traži riječ u opisu novog uvoza ili bankovne sinkronizacije i dodjeljuje kategoriju samo u aktivnom profilu.',
    noRulesBody:'Primjer: ako opis sadrži Uber, dodijelite Prijevoz. Pravilo vrijedi pri sljedećem uvozu; ne mijenja već spremljene transakcije.',
    changePasswordHint:'Mijenja lozinku lokalnog računa i odjavljuje njegove ostale sesije u ovom pregledniku. U demo načinu nije dostupno.',
    passwordUpdated:'Lozinka lokalnog računa je promijenjena. Ostale lokalne sesije su odjavljene.',
    twoFactorHint:'Dodatna provjera pri otključavanju lokalne sesije. SMS je samo demonstracija.',
    recoveryCodesHint:'Svaki kod vrijedi jednom ako izgubite pristup aplikaciji za kodove. Spremite ih izvan ovog preglednika; nakon zatvaranja više se ne prikazuju.',
    activeSessionsHint:'Samo prijave ovog korisnika u karticama i prozorima ovog preglednika. Drugi uređaji nisu uključeni.',
    ...
    smsMfaDescription:'Ova verzija ne šalje SMS. Demonstracijski kod prikazuje se u aplikaciji i vrijedi pet minuta.',
    sendSmsCode:'Prikaži demonstracijski kod', smsVerificationCode:'Demonstracijski SMS kod', sendDisableSmsCode:'Prikaži kod za isključivanje',
  ...
  Object.assign(translations.en, {
    settingsIntro:'Display, personal details, local protection and import rules.',
    settingsIntroClean:'Display, personal details, local protection and import rules.',
    baseCurrencyHint:'Changes the currency label and display format; does not convert amounts using exchange rates.',
    baseCurrencyInfo:'About the display currency',
    timezoneHint:'Sets today’s date and display time; entered transaction dates stay unchanged.',
    settingsLanguageHint:'Changes the interface language, not your data or custom category names.',
    hideBalancesHint:'Blurs on-screen amounts but does not encrypt them. Shortcut: Ctrl / ⌘ + Shift + H.',
    automationSubtitle:'A rule matches a word in a new import or bank-sync description and assigns a category only within the active profile.',
    noRulesBody:'Example: descriptions containing Uber go to Transport. The rule applies on the next import; saved transactions stay unchanged.',
    changePasswordHint:'Changes the local account password and signs out its other sessions in this browser. Unavailable in demo mode.',
    passwordUpdated:'Local account password changed. Other local sessions were signed out.',
    twoFactorHint:'An additional check when unlocking the local session. SMS is demonstration-only.',
    recoveryCodesHint:'Each code works once if you lose access to your authenticator app. Save them outside this browser; they disappear when this screen closes.',
    activeSessionsHint:'Only this user’s signed-in tabs and windows in this browser. Other devices are not included.',
    smsMfaDescription:'This version does not send SMS. The demonstration code is shown in the app and expires in five minutes.',
    sendSmsCode:'Show demonstration code', smsVerificationCode:'Demonstration SMS code', sendDisableSmsCode:'Show disable code',
  ...
  async function readImportFile(file) {
    try {
      ...
      if(/\.xlsx?$/i.test(file.name)){
        ...
        const workbook=XLSX.read(await file.arrayBuffer(),{type:'array',cellDates:true,sheetRows:MerImport.MAX_IMPORT_ROWS+2});const sheet=workbook.Sheets[workbook.SheetNames[0]];if(!sheet)throw new Error('empty-workbook');const rows=XLSX.utils.sheet_to_json(sheet,{header:1,raw:true,defval:''});const schema=MerImport.detectBankSchema(rows[0]||[],{sourceName:file.name});result=MerImport.parseRows(rows,state,{dateFormat:appState.settings.dateFormat,sourceName:file.name,profileId:appState.activeAccount,schema});
      }else if(/\.(xml|camt)$/i.test(file.name))result=MerAccounting.parseCamt053(await file.text(),state);
      else result=MerImport.parseBankImport(await file.text(),state,{dateFormat:appState.settings.dateFormat,sourceName:file.name,profileId:appState.activeAccount});
  ...
  function renderPremium() { applyPrivacy();renderGoals();if($('#bankSettingsModal').open)renderPremiumSettings();window.MerExportUI?.refresh();window.MerVaultsUI?.refresh();window.MerSavingsMinimal?.refresh();window.MerAnomalyUI?.refresh(); }
  ...
  window.MerVaultsBridge=Object.freeze({
    snapshot:()=>{const session=window.MerAuthProvider?.currentSession?.();return {profile:state,profileId:appState.activeAccount,language:currentLang,currency:appState.settings.currency,referenceDate:appReferenceDate,sessionId:session?`${session.userId}:${session.sessionId||session.issuedAt}`:'',authenticated:!$('#appShell').hidden&&!window.MerEnterpriseSecurity?.isLocked()&&!document.body.classList.contains('mfa-locked')};},
    mutate:(profileId,reason,action)=>{if(profileId!==appState.activeAccount)throw new Error('Profile changed');reactiveStore.update(reason,(_draft,profile)=>action(profile));},
    openModal,closeModal,openGoalEditor,formatMoney:value=>currency(value,true),toast:showToast,
