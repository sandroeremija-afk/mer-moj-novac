(function quickTools(){
  ...
  function resetCalculator(){input.value='0';result.textContent='0';result.classList.remove('is-error');input.setSelectionRange?.(1,1);}
  ...
  dialog.querySelector('.calculator-keys').addEventListener('click',event=>{if(!syncCalculatorAccess()||!dialog.open)return;const key=event.target.closest('[data-calculator-key]')?.dataset.calculatorKey;if(!key)return;if(key==='='){evaluate();return;}if(key==='C'){resetCalculator();}else{const replaceZero=input.value==='0'&&/^[0-9(]$/.test(key);const start=replaceZero?0:input.selectionStart??input.value.length,end=replaceZero?1:input.selectionEnd??start;input.setRangeText(key==='⌫'?'':key,key==='⌫'&&start===end?Math.max(0,start-1):start,end,'end');input.value=input.value.slice(0,120);result.textContent='';result.classList.remove('is-error');}input.focus({preventScroll:true});});
  ...
  trigger.addEventListener('click',()=>{render();if(!syncCalculatorAccess())return;resetCalculator();openModal(dialog);trigger.setAttribute('aria-expanded','true');position();input.focus({preventScroll:true});});
