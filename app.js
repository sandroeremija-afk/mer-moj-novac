...
Object.assign(translations.hr,{budgetOf:'{percent}% potrošeno · fleksibilni budžet {budget}',budgetNearPercent:'Blizu granice — {percent}% potrošeno',budgetAlmostPercent:'Granica gotovo dosegnuta — {percent}% potrošeno',budgetExceededPercent:'Budžet premašen — {percent}% potrošeno',budgetAlertsGroupedBody:'Dodirnite za pregled svake kategorije.',impactPrompt:'Unesite iznos da provjerimo utjecaj na proračun',highGoalContribution:'Ovaj cilj zahtijeva vrlo visoku mjesečnu uplatu — razmislite o produljenju roka'});
Object.assign(translations.en,{budgetOf:'{percent}% spent · flexible budget {budget}',budgetNearPercent:'Close to limit — {percent}% spent',budgetAlmostPercent:'Limit almost reached — {percent}% spent',budgetExceededPercent:'Budget exceeded — {percent}% spent',budgetAlertsGroupedBody:'Select to review each category.',impactPrompt:'Enter an amount so we can check its budget impact',highGoalContribution:'This goal requires a very high monthly contribution — consider extending the deadline'});
...
const personalDefaults = {
  ...
  goalBuckets:[{id:'goal-personal-reserve',name:'Fond za hitne slučajeve',target:10000,current:6240,dueDate:'2027-02-28',icon:'◎',primary:true,roundUpsEnabled:true},{id:'goal-personal-laptop',name:'Novi laptop',target:2400,current:0,dueDate:'2027-08-31',icon:'N',primary:false,roundUpsEnabled:false}],
...
function repairDemoGoalSeed() {
  const profile=appState.accounts.personal;
  const goals=Array.isArray(profile?.goalBuckets)?profile.goalBuckets:[];
  const laptopSeed=personalDefaults.goalBuckets.find(goal=>goal.id==='goal-personal-laptop');
  if(!profile||!laptopSeed)return false;
  let changed=false;
  const malformedDemoGoal=goals.find(goal=>/^moze$/i.test(String(goal.name||'').trim())&&Number(goal.target)>=10000);
  if(malformedDemoGoal){Object.assign(malformedDemoGoal,{name:laptopSeed.name,target:laptopSeed.target,current:laptopSeed.current,dueDate:laptopSeed.dueDate,icon:laptopSeed.icon,primary:false,roundUpsEnabled:false});changed=true;}
  const hasLaptop=goals.some(goal=>goal.id===laptopSeed.id||String(goal.name||'').trim().toLocaleLowerCase('hr')===laptopSeed.name.toLocaleLowerCase('hr'));
  if(!hasLaptop&&goals.length===1){goals.push(structuredClone(laptopSeed));changed=true;}
  return changed;
}
window.MerDemoData={repairGoalSeed:repairDemoGoalSeed};
...
function renderOverview() {
  const plan = getPlan();
  const rawPercent = Math.max(0,Number(plan.spentPercent)||0);
  ...
  $('#budgetProgress').style.width = `${Math.min(100, rawPercent)}%`;
  $('#budgetProgressTrack').setAttribute('aria-valuenow', Math.min(100, percent));
  $('#budgetProgressTrack').setAttribute('aria-valuetext',t('budgetOf',{percent,budget:currency(plan.monthlyBudget,true)}));
  const totalThreshold=MerCore.budgetThreshold(state.spent,plan.monthlyBudget);
  $('#budgetProgressTrack').className=`progress-track threshold-${totalThreshold.level}${rawPercent>=100?' over-cap':''}`;
...
function levelClass(percent) { return `threshold-${MerCore.budgetThreshold(percent,100).level}`; }
function thresholdMessage(percent) {
  const displayPercent=Math.round(Math.max(0,Number(percent)||0));
  const key={near:'budgetNearPercent',almost:'budgetAlmostPercent',exceeded:'budgetExceededPercent'}[MerCore.budgetSeverity(percent)];
  return key?t(key,{percent:displayPercent}):'';
}
...
function renderBudgetLists() {
  ...
  $('#budgetList').innerHTML = overviewCategories.map(cat => {
    const rawPct = state.derived?.categoryMetrics?.[cat.id]?.percent ?? MerCore.budgetThreshold(cat.spent,cat.limit).percent;
    const pct = Math.round(rawPct);
    const meta = categoryVisual(cat);
    const status=thresholdMessage(rawPct),atLimit=rawPct>=100;
    return `<div class="budget-item${atLimit?' is-over-budget':''}"><div class="budget-item-header"><span class="category-icon ${meta.className}">${escapeHtml(meta.icon)}</span><div><div class="budget-item-title"><strong>${escapeHtml(categoryName(cat.id))}</strong><span>${t('usedOf',{spent:currency(cat.spent,true),limit:currency(cat.limit,true)})}</span></div><div class="budget-bar${atLimit?' over-cap':''}"><span class="${levelClass(rawPct)}" style="width:${Math.min(100,rawPct)}%"></span></div>${rawPct>=80?`<small class="threshold-warning ${atLimit?'is-red overage-label':''}">${status}</small>`:''}</div><span class="budget-percent">${pct}%</span></div></div>`;
  }).join('');
}
...
function renderBudgetView() {
  ...
  const allocationRawPercent = plan.monthlyBudget>0?MerCore.ratioPercent(allocated,plan.monthlyBudget):allocated>0?100:0;
  const allocationPercent = Math.round(allocationRawPercent);
  ...
  $('#allocationProgress').style.width = `${Math.min(100,allocationRawPercent)}%`;
  const allocationThreshold=MerCore.budgetThreshold(allocationRawPercent,100),allocationBar=$('.allocation-bar');
  allocationBar.className=`allocation-bar threshold-${allocationThreshold.level}${allocationRawPercent>=100?' over-cap':''}`;
  allocationBar.setAttribute('aria-valuenow',String(Math.min(100,allocationPercent)));
  allocationBar.setAttribute('aria-valuetext',t('allocationPercent',{percent:allocationPercent}));
...
function budgetCategoryPercent(cat) { return state.derived?.categoryMetrics?.[cat.id]?.percent ?? MerCore.budgetThreshold(cat.spent,cat.limit).percent; }

function budgetCategoryRow(cat, manager=false) {
  const rawPct=budgetCategoryPercent(cat),pct=Math.round(rawPct),meta=categoryVisual(cat),remaining=Math.max(0,cat.limit-cat.spent),name=categoryName(cat.id),safeId=escapeHtml(cat.id),safeName=escapeHtml(name),atLimit=rawPct>=100,status=rawPct>=80?thresholdMessage(rawPct):`${currency(remaining,true)} ${currentLang==='hr'?'preostalo':'remaining'}`;
  return `<div class="budget-row${manager?' budget-manager-row':''}${atLimit?' is-over-budget':''}"><div class="budget-category"><span class="category-icon ${meta.className}">${escapeHtml(meta.icon)}</span><div><strong>${safeName}</strong><small class="${rawPct>=80?'threshold-warning':''}${atLimit?' is-red overage-label':''}">${status}</small></div></div><div class="budget-row-progress"><div class="budget-bar${atLimit?' over-cap':''}"><span class="${levelClass(rawPct)}" style="width:${Math.min(100,rawPct)}%"></span></div><span class="budget-percent">${pct}%</span></div><div class="budget-row-value">${currency(cat.spent,true)} / ${currency(cat.limit,true)}</div><button type="button" class="icon-button small edit-budget" data-edit-budget="${safeId}" aria-label="${currentLang==='hr'?'Uredi budžet za':'Edit budget for'} ${safeName}"><svg aria-hidden="true"><use href="#icon-edit"></use></svg></button></div>`;
}
...
function groupedBudgetNotificationTitle(count) {
  if(currentLang==='en')return `${count} categories are close to their limits`;
  const lastTwo=count%100,last=count%10,noun=lastTwo>=12&&lastTwo<=14?'kategorija':last>=2&&last<=4?'kategorije':'kategorija';
  return `${count} ${noun} ${noun==='kategorije'?'su':'je'} blizu limita`;
}
...
function buildNotifications() {
  ...
  const budgetAlerts=state.categories.flatMap(cat=>{const threshold=MerCore.budgetThreshold(cat.spent,cat.limit);return threshold.percent>=80?[{key:`budget:${cat.id}`,priority:threshold.level==='red'?4:3,type:threshold.level==='red'?'danger':'warning',icon:'icon-wallet',title:t('alertBudgetTitle'),body:t('alertBudgetBody',{category:categoryName(cat.id),percent:Math.round(threshold.percent)}),action:t('reviewBudget'),view:'budgets'}]:[];});
  MerCore.groupBudgetAlerts(budgetAlerts).forEach(item=>push(item.grouped?{...item,icon:'icon-wallet',title:groupedBudgetNotificationTitle(item.children.length),body:t('budgetAlertsGroupedBody'),action:t('reviewBudget'),view:'budgets'}:item));
...
function renderNotifications() {
  ...
  $('#notificationList').innerHTML=notifications.length?notifications.map((item,index)=>{const copy=item.grouped?`<details class="notification-budget-group"><summary><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.body)}</p></summary><ul>${item.children.map(child=>`<li>${escapeHtml(child.body)}</li>`).join('')}</ul></details>`:`<strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.body)}</p>`;return `<article class="notification-item"><span class="notification-symbol ${item.type}"><svg aria-hidden="true"><use href="#${item.icon}"></use></svg></span><div class="notification-copy">${copy}<button type="button" class="link-button" data-notification-view="${item.view}" data-notification-review="${item.reviewOnly?'true':'false'}" data-notification-index="${index}">${escapeHtml(item.action)}<span aria-hidden="true">→</span></button></div></article>`;}).join(''):`<div class="notification-empty">${t('noNotifications')}</div>`;
...
function renderInsights() {
  ...
  const palette=['var(--dark-green)','var(--blue)','var(--green)','var(--orange)','var(--red)','var(--purple)','var(--teal)'];
...
function resetTransactionCheck() {
  ...
  $('#spendCheck').innerHTML=`<svg aria-hidden="true"><use href="#icon-shield"></use></svg><div><strong>${t('impactPrompt')}</strong></div>`;
}
