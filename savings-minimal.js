(function initializeSavingsMinimal(root) {
  ...
  function renderHistory() {
    ...
    el('savingsHistoryBreakdown').innerHTML = `<h3>${say('Mjesečne uplate', 'Monthly contributions')}</h3><ul class="savings-detail-months">${months || `<li>${say('Nema povijesti za prikaz.', 'No history to show.')}</li>`}</ul><p class="savings-detail-note">${say('Mjesečni sažeci nisu stanje štednje: zbroj pokriva prikazane mjesece, a prosjek uključuje i mjesece bez uplata.', 'Monthly summaries are not your savings balance: the total covers the displayed months, and the average includes months without contributions.')}</p><p class="savings-history-record-count">${detail.entries.length} ${say('zapisa o štednji u ovom profilu.', 'savings records in this profile.')}</p>`;
  ...
  function renderGoal() {
    ...
    goalDialog.querySelector('.savings-detail-body').innerHTML = `<div class="savings-detail-goal"><h3>${esc(goal.name)}</h3><div class="savings-detail-amounts"><strong data-money>${money(goal.current)}</strong><span>${say('od', 'of')} ${money(goal.target)}</span></div><div class="savings-detail-progress" role="progressbar" aria-label="${esc(goal.name)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(percent)}"><span style="width:${percent}%"></span></div><p>${Math.round(percent)}% ${say('cilja ostvareno', 'of goal completed')}</p></div><dl class="savings-detail-metrics"><div><dt>${say('Još je potrebno', 'Remaining')}</dt><dd data-money>${money(remaining)}</dd></div><div><dt>${say('Mjesečno do roka', 'Monthly until target')}</dt><dd data-money>${metrics?.monthlyRequired == null ? '—' : money(metrics.monthlyRequired)}</dd></div><div><dt>${say('Ciljani datum', 'Target date')}</dt><dd>${esc(scheduled)}</dd></div><div><dt>${say('Zaokruživanje', 'Round-ups')}</dt><dd>${goal.roundUpsEnabled ? say('Uključeno', 'Enabled') : say('Isključeno', 'Disabled')}</dd></div></dl><p class="savings-detail-note">${say('Preostali iznos dijeli se s mjesecima do roka: oko 30,44 dana po mjesecu, zaokruženo naviše na najmanje 1 mjesec. Prinos nije uključen; pregled ne mijenja stanje ni plan.', 'The remaining amount is divided by months until the target: about 30.44 days per month, rounded up to at least 1 month. No returns are included; this preview does not change balances or your plan.')}</p><button type="button" class="secondary-button" id="savingsDetailAutomation">${say('Pravila štednje', 'Savings rules')}</button><h3>${say('Uplate u ovaj cilj', 'Deposits to this goal')}</h3>${entriesMarkup(detail.entries, detail.goals)}`;
