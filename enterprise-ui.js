(function enterpriseInterface() {
  ...
  function commands() {
    return [
      ...
      {label:copy('Mjesečni osvrt','Monthly review — Financial Wrapped'),run:()=>window.MerEngagementUI?.openWrapped()},
...
{label:copy('Tema aplikacije — svijetla / tamna','App theme — Light / Dark'),run:()=>openSetting('general','themeToggle')},
      {label:copy('Skeniraj i poveži račun','Scan receipt — OCR and matching'),run:()=>window.MerReceiptUI?.open()},
