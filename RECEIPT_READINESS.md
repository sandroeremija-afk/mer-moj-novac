...
2. The browser decodes/re-encodes the image locally, strips original metadata, resizes to at most 2048 px and limits the transmitted image to 2 MiB. It only calls the server after the user checks the explicit the prior AI provider consent box.
3. `/api/receipt` sends **only the consented image and extraction instruction** to the prior AI provider. It never sends the transaction list. The key is read server-side from `LEGACY_AI_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY`, with `LEGACY_RECEIPT_MODEL` then `LEGACY_AI_MODEL` controlling the model. No browser-prefixed key is used. `store:false` prevents Interactions history storage; this is not a claim about all provider retention policies.
...
- Live synthetic-only evaluation: `scripts/eval-receipt-live.js` generated a fictional receipt and invoked the real server handler using the existing local the prior AI provider configuration (`legacy-model`). HTTP 200 returned merchant `MER TEST TRGOVINA`, date `2026-09-07`, total `1250` cents and both correct item totals `450` + `800` cents. No private receipt or account data was used.
