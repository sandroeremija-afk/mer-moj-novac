...
test('cycle 2: header keeps only Date, notifications and the direct bank-dialog trigger',()=>{
  const title=html.indexOf('id="contextHeader"'),actions=html.indexOf('class="top-actions"',title),date=html.indexOf('id="systemDate"',actions),cluster=html.indexOf('class="header-action-cluster"',date),notification=html.indexOf('class="notification-wrap"',cluster),bank=html.indexOf('id="headerBankButton"',notification);assert.ok(title<actions&&actions<date&&date<cluster&&cluster<notification&&notification<bank);const topbar=html.slice(html.indexOf('<header class="topbar">'),html.indexOf('</header>'));assert.doesNotMatch(topbar,/id="(?:layoutEditToggle|themeToggle|settingsLanguage)"|class="language-switch"|data-lang=/);assert.match(topbar,/id="headerBankButton"[^>]*aria-controls="connectedBanksModal"[^>]*aria-haspopup="dialog"/);const generalStart=html.indexOf('data-settings-panel="general"'),generalEnd=html.indexOf('data-settings-panel="security"',generalStart),general=html.slice(generalStart,generalEnd);for(const id of ['settingsLanguage','themeToggle','layoutEditToggle'])assert.match(general,new RegExp(`id="${id}"`));assert.doesNotMatch(html,/id="activeModuleTitle"|id="systemTime"/);assert.match(app,/function renderSystemDate\(now=new Date\(\)\)/);assert.match(app,/setInterval\(\(\)=>\{renderSystemDate\(new Date\(\)\);renderBankSyncStatus\(\);\},60000\)/);assert.doesNotMatch(app,/timeLabel/);assert.match(app,/dashboardGreeting:'Dobro jutro, Mer'/);assert.match(app,/title\.textContent=t\('dashboardGreeting'\)/);assert.match(app,/\$\('#headerBankButton'\)\.addEventListener\('click',openBankSettings\)/);assert.match(css,/\.system-datetime time \{[^}]*font-size:\.6875rem/);assert.match(css,/\.header-action-cluster \{[^}]*display:flex/);assert.match(css,/\.context-header h1 \{/);
});

test('cycle 2: Insights contains all four requested analytical visuals without a card scrollbar',()=>{
  ['categoryDonut','monthlyBarChart','expenseStructureSummary','savingsGauge'].forEach(id=>assert.match(html,new RegExp(`id="${id}"`)));assert.match(app,/expenseStructureMarkup\(MerInsights\.expenseStructure\(state,insightsTimeframe,reference\)/);assert.match(app,/MerAccounting\.monthSeries/);assert.match(css,/\.advanced-insights-grid/);assert.doesNotMatch(css,/\.advanced-insights-grid[^}]*overflow-y\s*:\s*auto/);
});
