...
test('cycle 2: header groups date, theme, notifications and language in the requested order',()=>{
  const title=html.indexOf('id="activeModuleTitle"'),actions=html.indexOf('class="top-actions"',title),date=html.indexOf('id="systemDate"',actions),cluster=html.indexOf('class="header-action-cluster"',date),theme=html.indexOf('id="themeToggle"',cluster),notification=html.indexOf('class="notification-wrap"',theme),language=html.indexOf('class="language-switch"',notification);assert.ok(title<actions&&actions<date&&date<cluster&&cluster<theme&&theme<notification&&notification<language);assert.doesNotMatch(html,/id="systemTime"/);assert.match(app,/function renderSystemDate\(now=new Date\(\)\)/);assert.match(app,/setInterval\(\(\)=>renderSystemDate\(new Date\(\)\),60000\)/);assert.doesNotMatch(app,/timeLabel/);assert.match(app,/MerCore\.greetingFor\(new Date\(\),currentLang/);assert.match(css,/\.system-datetime time \{[^}]*font-size:\.6875rem/);assert.match(css,/\.header-action-cluster \{[^}]*display:flex/);assert.match(css,/\.page-heading h1 \{ display:none; \}/);
});
