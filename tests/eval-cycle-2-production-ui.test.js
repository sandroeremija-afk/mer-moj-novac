...
test('cycle 2: header groups Date, layout, theme, notifications, language and bank in order',()=>{
  const title=html.indexOf('id="contextHeader"'),actions=html.indexOf('class="top-actions"',title),date=html.indexOf('id="systemDate"',actions),layout=html.indexOf('id="layoutEditToggle"',date),cluster=html.indexOf('class="header-action-cluster"',layout),theme=html.indexOf('id="themeToggle"',cluster),notification=html.indexOf('class="notification-wrap"',theme),language=html.indexOf('class="language-switch"',notification),bank=html.indexOf('id="headerBankButton"',language);assert.ok(title<actions&&actions<date&&date<layout&&layout<cluster&&cluster<theme&&theme<notification&&notification<language&&language<bank);assert.doesNotMatch(html,/id="activeModuleTitle"|id="systemTime"/);assert.match(app,/function renderSystemDate\(now=new Date\(\)\)/);assert.match(app,/setInterval\(\(\)=>\{renderSystemDate\(new Date\(\)\);renderBankSyncStatus\(\);\},60000\)/);assert.doesNotMatch(app,/timeLabel/);assert.match(app,/dashboardGreeting:'Dobro jutro, Sandro'/);assert.match(app,/title\.textContent=t\('dashboardGreeting'\)/);assert.match(css,/\.system-datetime time \{[^}]*font-size:\.6875rem/);assert.match(css,/\.header-action-cluster \{[^}]*display:flex/);assert.match(css,/\.context-header h1 \{/);
});
