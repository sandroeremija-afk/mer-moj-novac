...
(async()=>{
  ...
  let outbound;
  const handler=createCashflowHandler({env:{OPENAI_API_KEY:'test-key'},fetchImpl:async(url,options)=>{outbound={url,options};return new Response(JSON.stringify({choices:[{finish_reason:'stop',message:{content:'Očekujete tri redovita računa.'}}]}),{status:200,headers:{'Content-Type':'application/json'}});}});
  const success=await call(handler);
  assert.equal(success.statusCode,200); assert.equal(success.body.source,'openai');
  assert.equal(new Headers(outbound.options.headers).get('authorization'),'Bearer test-key');
  ...
  const limited=createCashflowHandler({env:{OPENAI_API_KEY:'test-key'},fetchImpl:async()=>new Response('{}',{status:429})});
  assert.equal((await call(limited)).body.source,'deterministic');
  const large=createCashflowHandler({env:{OPENAI_API_KEY:'test-key'},fetchImpl:async()=>new Response('x'.repeat(70000),{status:200,headers:{'Content-Type':'application/json'}})});
  assert.equal((await call(large)).statusCode,502);
  const timeout=createCashflowHandler({env:{OPENAI_API_KEY:'test-key'},timeoutMs:50,fetchImpl:async(url,options)=>new Promise((resolve,reject)=>options.signal.addEventListener('abort',()=>reject(Object.assign(new Error('Aborted'),{name:'AbortError'}))))});
  ...
  process.stdout.write('Cycle 2 cashflow API: sanitization, OpenAI transport, fallback, bounds, timeout and rate limit passed.\n');
