(function initializeEngagement() {
  ...
  function openHealth() {
    ...
    el('healthAiExplain').textContent=say('Zatraži OpenAI objašnjenje','Ask OpenAI to explain');el('healthAiExplain').disabled=false;
    el('healthAiConsent').textContent=say('Klikom šaljete OpenAI servisu samo zbirne iznose i numerirane limite, bez imena, IBAN-a ili pojedinačnih transakcija. Prijedlog limita izračunava Mer, a AI ga objašnjava.','Clicking sends only totals and numbered limits to OpenAI, without names, IBANs or individual transactions. Mer calculates the limits; AI explains the proposal.');
  ...
  el('healthAiExplain').addEventListener('click',async()=>{
    ...
    const result=E.health(state,options());el('healthAiExplain').disabled=true;el('healthAiResponse').textContent=say('OpenAI priprema objašnjenje…','OpenAI is preparing an explanation…');
    ...
    catch(error){if(error.name!=='AbortError'&&!request.signal.aborted&&aiRequest===request&&context===responseContext()&&revision===reactiveStore.getRevision()&&healthDialog.open)el('healthAiResponse').textContent=say('OpenAI trenutačno nije dostupan. Provjereni prijedlog limita ostaje dostupan bez AI-ja.','OpenAI is unavailable. The verified limit proposal remains available without AI.');}
