(function initializeReceiptUI(root) {
  ...
  let reviewSections, reviewLinePager;
  ...
  function renderUpload(message = '', nativeFallback = false) {
    stopCamera();
    dialog.innerHTML = `${heading(copy('Skenirajte i povežite račun','Scan and match a receipt'))}<div class="receipt-body receipt-upload-body"><p class="receipt-intro">${copy('Dodajte fotografiju, provjerite očitane podatke i povežite postojeću transakciju.','Add a photo, review the extracted details and link an existing transaction.')}</p><div class="receipt-drop" id="receiptDrop" tabindex="0" role="button" aria-label="${copy('Odaberite fotografiju računa','Choose a receipt photo')}">${previewUrl?`<img src="${x(previewUrl)}" alt="${copy('Odabrani račun','Selected receipt')}"><span>${x(receipt.fileName)}</span>`:`<strong>${copy('Povucite račun ovdje','Drop a receipt here')}</strong><span>${copy('ili odaberite fotografiju · JPEG, PNG, WebP','or choose a photo · JPEG, PNG, WebP')}</span>`}</div><div class="receipt-upload-actions"><button class="secondary-button" type="button" id="receiptChoose">${copy('Odaberi datoteku','Choose file')}</button><button class="secondary-button" type="button" id="receiptCamera">${copy('Fotografiraj račun','Take photo')}</button></div><p class="receipt-note receipt-ocr-note" id="receiptOcrNote">${copy('OpenAI očitava fotografiju. Bankovne transakcije ne šalju se; spremaju se samo potvrđeni podaci, ne fotografija.','OpenAI reads the photo. Bank transactions are not sent; only reviewed details are saved, not the photo.')}</p><input id="receiptFile" type="file" accept="image/jpeg,image/png,image/webp" hidden><input id="receiptCameraFile" type="file" accept="image/*" capture="environment" hidden><label class="receipt-check"><input type="checkbox" id="receiptConsent" aria-describedby="receiptOcrNote"><span>${copy('Dopuštam OpenAI očitavanje ove fotografije, koja može sadržavati osobne podatke.','I consent to OpenAI reading this photo, which may contain personal information.')}</span></label><p role="alert" data-receipt-error ${message?'':'hidden'}>${x(message)}</p><div id="receiptLoading" class="receipt-loading" hidden role="status"><span>${copy('Očitavam račun…','Reading receipt…')}</span><i></i><i></i><i></i></div></div><footer class="receipt-footer"><button class="secondary-button" type="button" data-receipt-close>${copy('Zatvori','Close')}</button><button class="secondary-button" type="button" id="receiptManual">${copy('Unesi ručno','Enter manually')}</button><button class="primary-button" type="button" id="receiptAnalyze" ${preparedImage?'':'disabled'}>${copy('Očitaj račun','Read receipt')}</button></footer>`;
    bindClose();
  ...
  async function analyze() {
    ...
    if(!dialog.querySelector('#receiptConsent').checked){dialog.querySelector('#receiptConsent').focus();return showError(copy('Za AI očitavanje potvrdite pristanak ili odaberite ručni unos.','Consent to AI extraction or choose manual entry.'));}
    ...
    try {
      ...
      if(!guard.current(token,profileId())||!current())return;
      if(!response.ok||payload.source!=='openai'||!payload.receipt)throw Object.assign(new Error('OCR'),{code:payload.error});
      receipt=R.normalizeReceipt({...payload.receipt,id:receipt.id,imageHash:receipt.imageHash,fileName:receipt.fileName,source:'openai'});
